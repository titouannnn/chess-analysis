...
export class PuzzlesComponent implements OnInit {
  ...
  winRateOptions = [
    ...
    { value: WinRateFilter.HIGH, label: 'Ouvertures avec haut taux de victoire' },
    { value: WinRateFilter.LOW, label: 'Ouvertures avec faible taux de victoire' }
  ];
  
  // Puzzles recommandés par ouverture
  recommendedPuzzles: { recommendedPoints: number; Rating: number; URL: string }[] = [];
  ...
  initializeUserData(): void {
    this.apiService.sortByGameType(this.apiService.ALL_GENRES);
    
    // Initialiser les intervalles de temps
    this.apiService.initTimeInterval();
    this.apiService.setTimeTinterval(this.apiService.ALL_TIME, this.apiService.DATENULL, this.apiService.DATENULL);
  ...
  loadPuzzles(): void {
    ...
    this.puzzleService.getPuzzles().subscribe({
      ...
      error: (error) => {
        ...
        this.errorMessage = 'Erreur lors du chargement des puzzles. Veuillez réessayer.';
        this.isLoading = false;
      }
    });
  }
...
applyWinRateFilter(): void {
  ...
  // Récupérer les ouvertures en fonction du taux de victoire
  const openingsToUse = this.getOpeningsByWinRate();
  
  if (openingsToUse.length === 0) {
    this.isLoading = false;
    return;
  }
  
  // On va stocker ici tous les puzzles recommandés
  let allRecommendedPuzzles: { recommendedPoints: number; Rating: number; URL: string; opening: string }[] = [];
  ...
  if (this.selectedOpening) {
    ...
    allRecommendedPuzzles = puzzles.map(p => ({ ...p, opening: this.selectedOpening }));
  } else {
    // Sinon, collecter les puzzles pour chaque ouverture recommandée
    for (let i = 0; i < Math.min(3, openingsToUse.length); i++) {
      const opening = openingsToUse[i];
      const formattedOpening = opening.replace(/ /g, '_');
      const puzzles = this.puzzleService.collectPuzzlesByOpening(formattedOpening);
      
      // Ajouter l'information d'ouverture à chaque puzzle
      const puzzlesWithOpening = puzzles.map(p => ({ ...p, opening }));
  ...
  // en cherchant dans la liste originale des puzzles
  this.filteredPuzzles = allRecommendedPuzzles.map(recPuzzle => {
    ...
    if (originalPuzzle) {
      // Si trouvé, retourner toutes les informations
      return originalPuzzle;
    } else {
      // Sinon, créer un puzzle avec l'information d'ouverture
      return {
        PuzzleId: puzzleId,
        Rating: recPuzzle.Rating,
        Themes: '', // On n'a pas cette info
        OpeningTags: recPuzzle.opening.replace(/ /g, '_'), // Convertir l'ouverture au format attendu
        GameUrl: '',
        FEN: ''
      };
    }
  ...
  getOpeningsByWinRate(): string[] {
    ...
    const openingsWithWinRate = this.openingsStats.map(opening => {
      ...
      const totalGames = 
        stats.WinAsWhite + stats.WinAsBlack + 
        stats.LooseAsWhite + stats.LooseAsBlack +
        stats.DrawAsWhite + stats.DrawAsBlack;
    ...
    const minGames = 3;
    const validOpenings = openingsWithWinRate.filter(o => o.totalGames >= minGames);
  ...
  getOpeningWinRate(openingTags: string): number {
    ...
    let openingStats = this.openingsStats.find(opening => 
      opening.nom.toLowerCase() === formattedOpening.toLowerCase()
    );
    ...
    if (!openingStats) {
      ...
      openingStats = this.openingsStats.find(opening => {
        ...
        return openingWords.some(word => statsName.includes(word) && word.length > 3);
      });
    }
  ...
  get paginatedPuzzles(): Puzzle[] {
    ...
    return this.filteredPuzzles.slice(startIndex, startIndex + this.itemsPerPage);
  }
