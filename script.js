...
async function getAllGamesONLINE() {
    try {
        const archiveResponse = await fetch(`https://api.chess.com/pub/player/${username}/games/archives`);
        if (!archiveResponse.ok) {
            throw new Error(`Erreur lors de la récupération des archives: ${archiveResponse.status}`);
        }
        ...
        for (const archiveUrl of archiveData.archives) {
            ...
            if (!response.ok) {
                throw new Error(`Erreur lors de la récupération des parties: ${response.status}`);
            }
            const data = await response.json();
            allGamesAllTypes.push(...data.games);
        }
        console.log(`Nombre total de parties pour ${username} : ${allGamesAllTypes.length}`);
    ...
    for (const game of allGames) {
        ...
        if (gameDate >= dateDebut && gameDate <= dateFin) {
            ...
            if (playerColor === color) {
                if (result === "1-0" && color === 1 || result === "0-1" && color === 0) {
                    win++;
                } else if (result === "0-1" && color === 1 || result === "1-0" && color === 0) {
                    lose++;
                } else {
                    draw++;
                }
...
function sortByGameType(type){
    switch(type){
        case BULLET:
            return allGames = allGamesAllTypes.filter(game => game.time_class === "bullet");
        case BLITZ:
            return allGames = allGamesAllTypes.filter(game => game.time_class === "blitz");
        case RAPID:
            return allGames = allGamesAllTypes.filter(game => game.time_class === "rapid");
        case CLASSIC:
            return allGames = allGamesAllTypes.filter(game => game.time_class === "classic");
        case ALL_GENRES:
